...
- **Round cap.** Two sends per (sender → recipient → topic), rolling 24h; the third is
  refused with the row name and the remedy (`task set-body`). Topic is the first task ident
  in the body, or the pair itself when there is none. Per direction, so a topic affords two
  full exchanges before it becomes a document.
- **Acknowledgement refusal.** A short body that is substantially "ack / agreed / taking it

...
- **`task merge-audit` sweeps all three repos** (`FIVE_GATE_REPOS`, so a fourth repo is config and not a patch), reports the repo per row and counts `ambiguous` separately. Its previous answer — "0 OPEN, 0 merged-red across the newest 250 closes" — was true for the CLI repo and was not licensed across the product. Re-run: **6 real findings** the single-repo sweep could not see, including api #17 and frontend #16 OPEN, and api #12/#13 merged RED. It also stops reporting DIVE-1874/1875 as `CLOSED` on a `#25` that is a 5dive-api number colliding with an old CLI one — the benign half of the same bug, previously excused by a footnote that only covered `unverified` and not a state that happened to resolve.

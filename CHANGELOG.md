...
## v0.19.9 — fix(objective): the planner could not see that its own task was dead (OSS-37)
...
## v0.19.9 — fix(council): a test seam that defaulted to a LIVE rail was messaging a real seat (DIVE-2914)
...
## v0.19.9 — fix(task): the verifier auto-pick says when it skips the QA rung, and a job title no longer evicts the QA agent (DIVE-2912)
...
## v0.19.9 — fix(council): the receipt now says WHICH silence a seat's abstention was (DIVE-2891)
...
## v0.19.9 — fix(council): an amendment ballot now carries the candidate it is about, and refuses to mint when it cannot (DIVE-2889)
...
## v0.19.9 — fix(task): `task park` on a recurring INSTANCE now says it is stopping the beat (DIVE-2877)
...
## v0.19.9 — fix(heartbeat): the recurring-stall ladder's second rung changes hands instead of raising volume (DIVE-2853)
...
## v0.19.9 — feat(task): a close that NAMES a version is compared against the INSTALLED artifact (DIVE-2835)
...
## v0.19.9 — fix(auth): `auth set` refuses to revert a custom endpoint instead of silently re-deriving it (DIVE-2809)
...
## v0.19.9 — fix(push,deploy): a dry-run no longer green-lights a gate the real write refuses (DIVE-2801)
...
## v0.19.9 — fix(push,task): a refusal must not recommend a remedy the code will refuse (DIVE-2801 clauses 2 and 3)
...
## v0.19.9 — fix(agent): audit the inter-agent send rail with the derived caller (DIVE-2797)
...
## v0.19.9 — fix(task): a FIRST close must carry a reason, on both verbs; a cancel no longer deletes a live human gate (DIVE-2773)
...
## v0.19.9 — feat(create): `--base-url` points a claude agent at any Anthropic-compatible endpoint (DIVE-2757)
...
## v0.19.9 — feat(auth): qwen becomes the fifth claude BYO tile, smoked with a live key before it shipped (DIVE-2756)
...
## v0.19.9 — feat(proof): the zero-human badge moves to a rolling 30-day window, with a label DERIVED from the data it actually has (DIVE-2745)
...
## v0.19.9 — fix(ci): re-derive the calibration baseline from CI, and probe the runner on BOTH sides of the corpus (DIVE-2736)
...
## v0.19.9 — fix(ci): the core-tier budget is spent in a RELATIVE unit, so a slow runner stops reading as a fat corpus (DIVE-2728)
...
## v0.19.9 — fix(task): verification depth is re-measured at delivery, from the paths the work touched (DIVE-2719)
...
## v0.19.9 — fix(heartbeat): a tier HOLD skips the held TASK, not the whole agent (DIVE-2716)
...
## v0.19.9 — fix(council): a failed `verify` is a verdict, not a silent crash (DIVE-2711)
...
## v0.19.9 — fix(merge-gate): a dead GitHub query no longer counts as a completed scan (DIVE-2705)
...
## v0.19.9 — fix(release): the changelog.d fold's deletions are part of the release commit (DIVE-2700)
...
## v0.19.9 — ci(full-sweep): report the nightly total in the units of the policy dial (DIVE-2669)
...
## v0.19.9 — fix(ci): full-sweep runs on every push to main, cost-bounded (DIVE-2667)
...
## v0.19.9 — feat(gate): the merge gate compares the merged sha against the sha the verifier says it graded (DIVE-2656)
...
## v0.19.9 — feat(ui): `5dive ui`, three read-only views served by the CLI itself (DIVE-2655)
...
## v0.19.9 — feat(task): every merge-gate accept now says merged is not deployed (DIVE-2641)
...
## v0.19.9 — feat(doctor): per-host CLI freshness, so a verifier can tell INSTALLED from MERGED (DIVE-2640)
...
## v0.19.9 — fix(agent): report registry fetch failures without claiming a slug is absent (DIVE-2600)
...
## v0.19.9 — fix(pack): preserve contractions when renaming imported personas (DIVE-2599)
...
## v0.19.9 — fix(task): the loop bounce decision is read from the DECISION SEGMENT, not from a substring over the whole answer (DIVE-2572)
...
## v0.19.9 — fix(push): a mint failure says which of four things went wrong (DIVE-2566)
...
## v0.19.9 — fix(task): the result guard keys on the COLUMN, not on whether the row is closed (DIVE-2483)
...
## v0.19.9 — fix(task): the preservation is now ANNOUNCED, and the seam is dated (DIVE-2483)
...
## v0.19.9 — fix(agent): do not confirm an unconfirmed send (DIVE-2362)
...
## v0.19.9 — fix(task): make delivery bindings observable in task show and list (DIVE-2316)

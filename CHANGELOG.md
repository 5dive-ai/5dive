...
- fix(cli): `5dive task --help` and top-level `5dive --help` no longer execute Markdown-backticked examples as shell command substitutions. The unquoted usage heredocs now escape literal backticks, eliminating the spurious `task: command not found` / `doctor: command not found` diagnostics and preventing help rendering from invoking privileged command paths (DIVE-1342).

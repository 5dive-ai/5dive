...
## v0.19.3 — fix(objective): the planner could not see that its own task was dead (OSS-37)
...
## v0.19.3 — fix(ci): the core-tier budget is spent in a RELATIVE unit, so a slow runner stops reading as a fat corpus (DIVE-2728)
...
## v0.19.3 — fix(task): verification depth is re-measured at delivery, from the paths the work touched (DIVE-2719)
...
## v0.19.3 — fix(heartbeat): a tier HOLD skips the held TASK, not the whole agent (DIVE-2716)
...
## v0.19.3 — fix(council): a failed `verify` is a verdict, not a silent crash (DIVE-2711)
...
## v0.19.3 — fix(merge-gate): a dead GitHub query no longer counts as a completed scan (DIVE-2705)
...
## v0.19.3 — fix(release): the changelog.d fold's deletions are part of the release commit (DIVE-2700)
...
## v0.19.3 — ci(full-sweep): report the nightly total in the units of the policy dial (DIVE-2669)
...
## v0.19.3 — fix(ci): full-sweep runs on every push to main, cost-bounded (DIVE-2667)
...
## v0.19.3 — feat(gate): the merge gate compares the merged sha against the sha the verifier says it graded (DIVE-2656)
...
## v0.19.3 — feat(ui): `5dive ui`, three read-only views served by the CLI itself (DIVE-2655)
...
## v0.19.3 — feat(task): every merge-gate accept now says merged is not deployed (DIVE-2641)
...
## v0.19.3 — feat(doctor): per-host CLI freshness, so a verifier can tell INSTALLED from MERGED (DIVE-2640)
...
## v0.19.3 — fix(task): the loop bounce decision is read from the DECISION SEGMENT, not from a substring over the whole answer (DIVE-2572)
...
## v0.19.3 — fix(push): a mint failure says which of four things went wrong (DIVE-2566)
...
## v0.19.3 — fix(task): the result guard keys on the COLUMN, not on whether the row is closed (DIVE-2483)
...
## v0.19.3 — fix(task): the preservation is now ANNOUNCED, and the seam is dated (DIVE-2483)
...
## v0.19.3 — fix(task): make delivery bindings observable in task show and list (DIVE-2316)

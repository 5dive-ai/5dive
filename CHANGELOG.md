...
- fix(openclaw): fresh agents now resolve a supported Node 24 runtime explicitly, including a stable `~/.local/bin/node` installer link and direct Node invocation for OpenClaw's `/usr/bin/env node` launcher during create-time model setup. Channel-less agents use an idempotent `config set gateway.mode local` headless bootstrap instead of blocking inside the interactive `openclaw configure` wizard before chat. The managed install/upgrade path installs `openclaw@latest` directly into the active Node 24 npm prefix (the upstream wrapper re-selects nvm's default Node and can attempt a privileged NodeSource upgrade), then links only that executable runtime (DIVE-1328).

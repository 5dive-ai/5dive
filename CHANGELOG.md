...
- fix(agent): `agent info` (and `agent list`'s model/effort resolution) no longer issues denied nested-sudo probes when run bare by a scoped agent identity. Runtime model/effort settings are now read directly when the caller can access them (root still sees every agent; an agent sees its own), unreadable sibling settings remain unknown, and non-root CLI-version probes execute the already-shared binary directly instead of attempting `sudo -u claude`. This removes the three `sudo command-not-allowed` audit entries a single Claude `agent info` previously generated without widening the scoped sudo allowlist (DIVE-1343).

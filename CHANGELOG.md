...
## v0.19.32 — fix(task): `assignee` / `verifier` / `created_by` must name a real agent (DIVE-3344)
...
## v0.19.32 — fix(agent config): buzz had a staging GATE and no install DISPATCH (DIVE-3333)
...
## v0.19.32 — test(task): the open-row announcement's STREAM is graded, not documented (DIVE-2748)
...
## v0.19.32 — fix(agent): `agent info` reports whether a seat is TRANSACTING, not only whether it is up (DIVE-3274)
...
## v0.19.32 — fix(gate): route a ship gate on the ROW'S BRANCH BINDING, not on the ask's prose, and say out loud when a gate did not route at all (DIVE-3266)
...
## v0.19.32 — fix(task): the merge-gate asserts its OWN instrument, and names the seat where it is inert (DIVE-1935)

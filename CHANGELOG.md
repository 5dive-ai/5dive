...
## 0.10.13 — new-work generator: idle becomes a trigger, not a resting state (STEER-11) (2026-07-19)

- feat(steer): `5dive steer propose` + a heartbeat trigger that mints fresh work when the fleet drains to ZERO dispatchable tasks, so "self-steering" no longer collapses into "runs a queue, then idles" (STEER-5 design; companion to STEER-1 idle-watchdog and STEER-4 delegated push). Sourcing is RULE-BASED on real, auditable signal only — active projects whose goal is unrealized but have no open work, plus ROADMAP.md unchecked items — and every candidate carries its `steer-source:` (a candidate with no traceable source is never minted; no free-invention). Candidates are filed as REVIEW-STATE tasks (blocked behind an `approve|revise` decision gate, assignee pinned to the project lead, `created_by=steer-generator` for audit), NEVER auto-dispatched to a builder; on lead-approve, `_hb_steer_apply_sweep` flips the candidate to a dispatchable todo for the intended assignee. Guardrails: per-cycle cap (`STEER_MAX_PROPOSE`, default 3), outstanding cap across all unreviewed candidates (`STEER_MAX_OUTSTANDING`, default 5), and de-dup by source so one signal is proposed at most once. The trigger (`_hb_steer_sweep`) fires only when the board is FULLY drained (0 open standard tasks fleet-wide) for N consecutive ticks (`STEER_IDLE_TICKS`, default 2) — a merely-*dammed* queue (open-but-blocked work) is STEER-1's lane and is left alone — and is idempotent per drain episode. New `tests/steer_propose_unit.sh` (15/15): drain→mint, review-state routing, source-grounding + audit stamp, de-dup, approve→dispatch, debounce, episode-idempotency, dammed-negative, per-cycle + outstanding caps.

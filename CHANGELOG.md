...
- fix(openclaw): fresh agents now resolve a supported Node 24 runtime explicitly, including a stable `~/.local/bin/node` installer link and direct Node invocation for OpenClaw's `/usr/bin/env node` launcher during create-time model setup. Channel-less agents use an idempotent `config set gateway.mode local` headless bootstrap instead of blocking inside the interactive `openclaw configure` wizard before chat. `agent install openclaw --upgrade` also removes the old cross-version launcher before reinstalling, so switching npm to Node 24 cannot replace a working launcher with a dangling symlink into an empty global tree (DIVE-1328).

...
- fix(gates): a decision/approval gate a MAKER files on a maker→verifier loop now routes to the loop's VERIFIER agent (via `5dive agent send`), not the paired human — closes the CNCL-9 pain where `task need --type=decision` addressed to main-the-verifier pinged lodar instead. Routing is intrinsic to the loop KIND (bypasses the `gate_builder_routing` pref, like eng-ship/curation); the true-human tier-2 floor (money/comms/secrets/destructive/brand) still wins, and a gate filed BY the verifier (e.g. the max-iters escalation) never self-routes. Also: `task reject` now SUPERSEDES any still-open need-gate on the bounced task (marks it `auto:reject`), so the DIVE-1490 re-nag ladder stops firing a question the rejection already mooted (DIVE-1495).

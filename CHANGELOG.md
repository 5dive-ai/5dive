...
- fix(init): Antigravity authentication in `5dive init` now uses the detached OAuth session flow, prints the captured Google URL, accepts the callback code, detects the written credential, and resumes agent creation automatically instead of trapping the user inside agy's TUI until they discover `/exit`. Direct `agent auth login antigravity` keeps its indefinite interactive fallback but now explicitly tells users to type `/exit` after sign-in (DIVE-1339).

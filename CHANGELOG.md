...
## 0.15.15 — fix(task): three gate carve-outs silently overrode an explicit `--tier=2` hard-human pin, routing brand/money calls to an agent (DIVE-1957) (2026-07-25)
...
- **`tests/gate_tier2_explicit_pin_unit.sh`** varies the **TITLE** as well as the ask — every veto case appears twice, once with the keyword in the ask and once with a neutral ask and the keyword only in the title. A suite that varied only the ask would have passed on the unfixed code, since the title axis was never exercised. Mutation-checked against `origin/main`'s `cmd_task.sh`: **8 of 15 red before the fix, 15/15 after**, and the un-pinned counterpart of each class is asserted to still downgrade so the suite cannot be satisfied by disabling a carve-out.

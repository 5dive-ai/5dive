...
## v0.19.42 — fix(cli): a wrapped tool's non-zero is reported as that tool's status, and an unfilable `5dive bug` report is spooled instead of lost (DIVE-2792)
...
- **`5dive agent logs`** was the second instance, found by sweeping every arm of the dispatch
  ...
  bug banner; in `--json` mode `journalctl | jq` printed jq's `{ok:true}` envelope with zero
  lines **before the failure was knowable**, and then the backstop's `{ok:false}` after it — two
  contradictory envelopes on one stdout, the first of which a consumer reading the first object
  would take as a successful empty read. Now the status is captured, the envelope is only printed
  when the read succeeded, and a failure says whose status it is (`class:"passthrough"`).
  A successful read that returns nothing reports `lines: []`: the capture is fed through a
  here-string, and `<<<""` is one empty line to `jq -R … inputs`, not zero input — an agent whose
  unit has not logged yet is the ordinary case for that.
...
## v0.19.42 — fix(self-update): skip an agent holding an in_progress row and bounce it at its next task boundary (DIVE-3173)
...
## v0.19.42 — feat(task): `merge-unverified` reads back the closes the merge gate could not check (DIVE-3526)
...
## v0.19.42 — fix(branch-hygiene): the weekly digest classifies branches by EVIDENCE, not age (DIVE-2394)
...
## v0.19.42 — fix(council): `authority.gate_clear_leads` can actually be set (DIVE-3493)
...
## v0.19.42 — feat(task): an inert push-for-review clears at filing, pinging nobody (DIVE-3481)
...
## v0.19.42 — fix(usage): the middle wildcard is a read too (DIVE-3419)
...
## v0.19.42 — fix(task): `assignee` / `verifier` / `created_by` must name a real agent (DIVE-3344)
...
## v0.19.42 — fix(agent config): buzz had a staging GATE and no install DISPATCH (DIVE-3333)
...
## v0.19.42 — test(task): the open-row announcement's STREAM is graded, not documented (DIVE-2748)
...
## v0.19.42 — fix(agent): `agent info` reports whether a seat is TRANSACTING, not only whether it is up (DIVE-3274)
...
## v0.19.42 — fix(gate): route a ship gate on the ROW'S BRANCH BINDING, not on the ask's prose, and say out loud when a gate did not route at all (DIVE-3266)
...
## v0.19.42 — fix(task): the merge-gate asserts its OWN instrument, and names the seat where it is inert (DIVE-1935)

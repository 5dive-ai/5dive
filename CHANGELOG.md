...
## v0.19.43 — fix(self-update): skip an agent holding an in_progress row and bounce it at its next task boundary (DIVE-3173)
...
## v0.19.43 — feat(task): `merge-unverified` reads back the closes the merge gate could not check (DIVE-3526)
...
## v0.19.43 — fix(branch-hygiene): the weekly digest classifies branches by EVIDENCE, not age (DIVE-2394)
...
## v0.19.43 — fix(council): `authority.gate_clear_leads` can actually be set (DIVE-3493)
...
## v0.19.43 — feat(task): an inert push-for-review clears at filing, pinging nobody (DIVE-3481)
...
## v0.19.43 — fix(usage): the middle wildcard is a read too (DIVE-3419)
...
## v0.19.43 — fix(task): `assignee` / `verifier` / `created_by` must name a real agent (DIVE-3344)
...
## v0.19.43 — fix(agent config): buzz had a staging GATE and no install DISPATCH (DIVE-3333)
...
## v0.19.43 — test(task): the open-row announcement's STREAM is graded, not documented (DIVE-2748)
...
## v0.19.43 — fix(agent): `agent info` reports whether a seat is TRANSACTING, not only whether it is up (DIVE-3274)
...
## v0.19.43 — fix(gate): route a ship gate on the ROW'S BRANCH BINDING, not on the ask's prose, and say out loud when a gate did not route at all (DIVE-3266)
...
## v0.19.43 — fix(task): the merge-gate asserts its OWN instrument, and names the seat where it is inert (DIVE-1935)

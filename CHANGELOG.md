...
## v0.23.5 — feat(task): gate state on the surfaces people READ — `ls` column, `show` header, `--gated` (DIVE-3785)
...
## v0.23.5 — feat(task): `5dive task doctor` — every open row nothing will dispatch, and why (DIVE-3784)
...
## v0.23.5 — feat(liveness): `5dive liveness` — a seat is alive only against an artifact it WROTE (DIVE-3778)
...
## v0.23.5 — feat(supervisor): rung 4 — a poller-dead seat is restarted, once per 6h (DIVE-3753)
...
## v0.23.5 — feat(durable): an irreversible action fires ONCE, even when the agent crashes mid-flight (INST-8)
...
## v0.23.5 — fix(memory): consolidate reports what it PRODUCED, and the sweep can now authenticate (DIVE-3711)
...
## v0.23.5 — feat(memory): `5dive memory consolidate` — async transcript → memory atoms (DIVE-3628, DIVE-726 phase 1)
...
## v0.23.5 — fix(self-update): skip an agent holding an in_progress row and bounce it at its next task boundary (DIVE-3173)
...
## v0.23.5 — feat(task): `merge-unverified` reads back the closes the merge gate could not check (DIVE-3526)
...
## v0.23.5 — fix(branch-hygiene): the weekly digest classifies branches by EVIDENCE, not age (DIVE-2394)
...
## v0.23.5 — fix(council): `authority.gate_clear_leads` can actually be set (DIVE-3493)
...
## v0.23.5 — feat(task): an inert push-for-review clears at filing, pinging nobody (DIVE-3481)
...
## v0.23.5 — fix(usage): the middle wildcard is a read too (DIVE-3419)
...
## v0.23.5 — fix(task): `assignee` / `verifier` / `created_by` must name a real agent (DIVE-3344)
...
## v0.23.5 — fix(agent config): buzz had a staging GATE and no install DISPATCH (DIVE-3333)
...
## v0.23.5 — test(task): the open-row announcement's STREAM is graded, not documented (DIVE-2748)
...
## v0.23.5 — fix(agent): `agent info` reports whether a seat is TRANSACTING, not only whether it is up (DIVE-3274)
...
## v0.23.5 — fix(gate): route a ship gate on the ROW'S BRANCH BINDING, not on the ask's prose, and say out loud when a gate did not route at all (DIVE-3266)
...
## v0.23.5 — fix(task): the merge-gate asserts its OWN instrument, and names the seat where it is inert (DIVE-1935)

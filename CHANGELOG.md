...
- **the seal is untouched.** `canonicalTranscript` seals seat/vote/rationale only, so every added field is additive and pre-DIVE-1869 receipts still verify byte-identically — pinned by an assertion rather than asserted in a comment.

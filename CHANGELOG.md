...
## 0.13.14 — account list: report a claude BYO provider so the dashboard can draw its sub-badge (DIVE-1763) (2026-07-22)

- **fix(account): `account_signin_detail` now reports the resolved provider for a BYO-claude agent, so `5dive account list --json` emits `signins.claude.provider=<byo id>` and the `/dashboard/agents` provider sub-badge (frontend b9f05d97) finally has something to render.** Root cause: the `*)` default branch keyed sign-in detail off the `.credentials.json` sentinel, which a BYO-claude agent never writes — `_apply_byo_claude` instead points the runtime at a custom Anthropic-compatible endpoint via `ANTHROPIC_BASE_URL` in the profile's `combined.env` — so the branch bailed with `{}` and provider stayed null. New `claude)` case reverse-maps that stored base url against `CLAUDE_PROVIDER_BASEURL` (deepseek/moonshot/openrouter/zai) to the canonical provider id; a plain Anthropic subscription has no base url so provider stays null and no badge draws (correct). `signedInAt` falls back to `combined.env`'s mtime when the credential sentinel is absent. The `claude` type already surfaced via `account_types_authed` (empty `ANTHROPIC_API_KEY=`/`CLAUDE_CODE_OAUTH_TOKEN=` lines), so only provider extraction was missing. Split from DIVE-1762; unblocks quinn's DIVE-1761 Tier-B badge assertion.

...
## v0.19.2 — fix(heartbeat): a tier HOLD skips the held TASK, not the whole agent (DIVE-2716)
...
## v0.19.2 — fix(council): a failed `verify` is a verdict, not a silent crash (DIVE-2711)
...
## v0.19.2 — fix(merge-gate): a dead GitHub query no longer counts as a completed scan (DIVE-2705)
...
## v0.19.2 — fix(release): the changelog.d fold's deletions are part of the release commit (DIVE-2700)
...
## v0.19.2 — fix(ci): full-sweep runs on every push to main, cost-bounded (DIVE-2667)
...
## v0.19.2 — feat(gate): the merge gate compares the merged sha against the sha the verifier says it graded (DIVE-2656)
...
## v0.19.2 — feat(ui): `5dive ui`, three read-only views served by the CLI itself (DIVE-2655)
...
## v0.19.2 — feat(task): every merge-gate accept now says merged is not deployed (DIVE-2641)
...
## v0.19.2 — feat(doctor): per-host CLI freshness, so a verifier can tell INSTALLED from MERGED (DIVE-2640)

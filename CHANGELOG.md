...
## v0.18.4 — fix(task): `task done` died with empty output when the result named no branch (DIVE-2603)
...
## v0.18.4 — feat(agent): `agent list` reports credential health, so a lapsed seat stops rendering live (DIVE-1953)
...
## v0.18.4 — fix(push): the per-repo installation lookup could not fail, so its own fallback was dead code (DIVE-2566)
...
## v0.18.4 — fix(task): the mandatory merge-gate now catches a branch cited in prose, not just a PR (DIVE-2577)
...
## v0.18.4 — fix(task): `task deliver --result=` destroyed a closed row's result too (DIVE-2476)
...
cannot see.
## v0.18.4 — feat(pack): a marketplace pack imports onto codex and opencode, not only Claude (DIVE-2568)
...
## v0.18.4 — fix(digest): a completion was credited to whoever OWNED the row at close, which on a graded row is the verifier (DIVE-2556)
...
## v0.18.4 — feat(pack): export an agent as ONE AGENTS.md that codex and opencode read as-is (DIVE-2565)
...
## v0.18.4 — feat(a2a): stamp `via=` when the claimed sender and the measured caller diverge (DIVE-2552)
...
## v0.18.4 — fix(push): resolve the App installation PER REPO, not from one pinned id (DIVE-2563)
...
## v0.18.4 — fix(push): the workflow-scope probe fetched unauthenticated, so every private-repo push demanded `workflows:write` (DIVE-2547)
...
## v0.18.4 — feat(actor): `5dive whoami`, one sealed actor derivation (DIVE-2517)
...
## v0.18.4 — feat(task): merge-audit LABELS findings delivered-vs-cited, and never filters them (DIVE-1975)
...
## v0.18.4 — feat(broker): generalize the capability broker and fold in delegated deploy (INST-5)
...
## v0.18.4 — fix(task): refuse a close that would REPLACE an already-closed row's result (DIVE-2464)
...
## v0.18.4 — fix(release-cut): move the nightly off the contended top-of-hour and poll for CI to settle (DIVE-2466)
...
## v0.18.4 — fix(release): the release page says WHAT shipped, and the tag stamps the CHANGELOG (DIVE-2452)
...
## v0.18.4 — feat(gh): route agent GitHub writes through the machine account (DIVE-2448)
...
## v0.18.4 — feat(task): CHANNEL PROOF of the human's answer clears a tier-2 gate, so a decision already made in chat is not re-entered as a button tap (DIVE-2412)
...
## v0.18.4 — fix(task): pronoun options resolve to an account frame (DIVE-2212)
...
## v0.18.4 — feat(ledger): one append-only lifecycle log with the authority envelope (INST-4, phase A)
...
## v0.18.4 — feat(task): displaced gates have a reader with an honest coverage boundary (DIVE-2133)
...
## v0.18.4 — fix(up): a skill that FAILED to install is no longer summarised as `errors=0` (DIVE-2347)
...
## v0.18.4 — fix(doctor): a FAILED env-override report no longer reads as "no overrides set" (DIVE-2336)
...
## v0.18.4 — feat(doctor): REPORT the FIVE_* knobs in effect and configured (DIVE-2328/2327)
...
## v0.18.4 — fix(tests): harnesses no longer inherit the caller's product knobs (DIVE-2325)
...
## v0.18.4 — fix(gate): the merge gate stops reporting COULD-NOT-CHECK as NOT-MERGED (DIVE-2318)
...
## v0.18.4 — fix(loop): a spend read that FAILED is NOT-REACHED, not zero — and no longer clobbers the running total (DIVE-2304)
...
## v0.18.4 — fix(task): the tier-2 category floor stops reading `press` inside `suppression`, without dropping `$500` out of the un-appealable half (DIVE-2301)
...
## v0.18.4 — fix(tasks): the production task board refuses a write from a sourced-library caller, so a harness cannot leak fixture rows onto it (DIVE-2249)
...
## v0.18.4 — fix(heartbeat): the dispatcher claims the task it nudges, so the whole stuck-work recovery layer stops reading a dead field (DIVE-2244)
...
## v0.18.4 — fix(task): a failed open-instance read no longer forges a `last_skipped_at` suppression that never happened (DIVE-2273)
...
## v0.18.4 — fix(council): the veto principal is redacted where it is GENERATED, not per-file (DIVE-2278)
...
## v0.18.4 — feat(task): a gate can DECLARE that it needs a human, and stop being answered by whoever is grading the ticket (DIVE-2241)
...
## v0.18.4 — fix(council): the founder veto has never been exercisable — the hold window had already closed on all six offers ever sent (DIVE-2257)
...
## v0.18.4 — fix(task): a recurring template that the scheduler SKIPPED now says so, instead of reading exactly like one it never reached (DIVE-2237)
...
## v0.18.4 — fix(ask): a reply fence whose markers sit INLINE is now harvested, so a grok seat stops reading as a silent abstain (DIVE-2216)
...
## v0.18.4 — fix(heartbeat): an UNMEASURABLE tier no longer disables the privilege-escalation-by-queue guard (DIVE-2213)
...
## v0.18.4 — fix(a2a): the envelope's tier= field is now always stamped, with a reason when it cannot be measured (DIVE-2210)
...
## v0.18.4 — feat(digest): a 30-day window, with the aggregates it does NOT scope named out loud (DIVE-1921)
...
## v0.18.4 — fix(heartbeat/task): a verifier who filed a human gate has ACTED, and neither verb may resolve that gate by side effect (DIVE-2196)
...
## v0.18.4 — fix(gate): a gate escalates from the agent that FILED it, not from whoever created the task (DIVE-1945)
...
## v0.18.4 — feat(comms): the terse rule now bounds HOW OFTEN you send, and covers agent-to-agent (DIVE-2191)
...
## v0.18.4 — feat(task): the tier-2 floor says WHY it fired, and a design decision can appeal it on the record instead of by rewording (DIVE-2089)
...
## v0.18.4 — fix(proof): the daily publisher no longer dies on its own log, and a successful tick finally says so (DIVE-2044)
...
## v0.18.4 — fix(agent): typed sends REFUSE a credential/login pane, so an inter-agent message can no longer become the agent's API key (DIVE-2137, gh#214)
...
## v0.18.4 — fix(agent): the sudo-grant measurement can finally see a PEER, via one privileged read (DIVE-2135)
...
## v0.18.4 — fix(agent): `agent list` carries the measured sudo grant too, so the SURVEY surface stops reading as authoritative (DIVE-2088)

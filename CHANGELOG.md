...
- test(council): new `tests/council_dispatch_unit.mjs` (37 assertions — parse/blind-isolation/abstain/quorum-boundary/adversarial-separation/deterministic-synthesis) + CLI dispatch contract (real-agents default, `--standalone` seam). All three council harnesses are now gated in CI via `tests/council_unit.sh` (they previously ran locally only). Engine 57/57, CLI contract 19/19, dispatch 37/37. The live e2e (a real convene over 3+ seated agents) is run separately in a coordinated quiet window.

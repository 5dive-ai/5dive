...
## 0.15.23 — fix(heartbeat): the DIVE-2001 guard failed open SILENTLY, and its format contract was tested by nothing (DIVE-2003) (2026-07-25)

- **two different defects shared one test signature.** olivia measured it verifying DIVE-2001: reverting *only* the test stub to its old no-epoch format produces `8/2` — byte-identical to the signature of deleting the guard condition outright. The pre-ask guard fails open on an unparseable epoch (correct: withholding a legitimate flag is a silence), but it did so writing **no log line and no audit row**, so a guard that had stopped working looked exactly like one with nothing to skip. A silence inside a fix whose whole subject is silences. The fail-open branch now logs and audits, naming which side was unparseable.
- **the production format string was exercised by no test at all.** `heartbeat_gate_shipped_unit.sh` stubs `_hb_repo_grep_ident`, so `--format='%h %ct %s'` had zero coverage, and the sweep's `awk '{print $3}'` is only correct because that function *prepends* the repo stem — a two-function contract with nothing asserting it. New **hermetic** `heartbeat_gate_shipped_contract_unit.sh` git-inits a throwaway repo in its own tmpdir, calls the REAL function, and asserts field 1 is the repo stem and field 3 is a numeric epoch. No network, no configured repo.
- **it also pins `%ct` over `%at`.** With author-date, an old-authored commit merged *after* the ask would be wrongly skipped — another silence. The fixture's author and committer dates differ by 42s so the wrong one cannot pass.
- mutation-proven, including the point of the whole exercise: reverting the format to `%h %s` fails the contract test (3/2) **while the stubbed suite still reports 10/10**. `%at` fails 1. Dropping the repo prepend fails 3. Removing the fail-open log fails the says-so assertion.

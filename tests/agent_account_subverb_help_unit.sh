...
UNDOC=()
for lab in _self_restart _default_skills _sync_codex_baseline _reconcile_sudoers; do
  rc="$(run agent "$lab" --help)"
  { [[ "$rc" != "0" ]] && has "$(err)" 'no usage text'; } || UNDOC+=("$lab(rc=$rc)")
done
(( ${#UNDOC[@]} == 0 )) \
  && ok_t "C1: the 4 undocumented internals are refused BY NAME, not answered with an invented line" \
  || bad_t "C1: an undocumented verb is refused by name" "${UNDOC[*]}"
C_GREW=()
for pair in "${AGENT_LABELS[@]}"; do
  read -r lab _ <<<"$pair"
  case "$lab" in _self_restart|_default_skills|_sync_codex_baseline|_reconcile_sudoers) continue ;; _*) ;; *) continue ;; esac
  rc="$(run agent "$lab" --help)"
  [[ "$rc" == "0" ]] || C_GREW+=("$lab(rc=$rc)")
done
(( ${#C_GREW[@]} == 0 )) \
  && ok_t "C2: ... and every OTHER internal does answer, so C1's list is exactly 4 and not 'whatever failed'" \
  || bad_t "C2: the undocumented list is exactly those 4" "these also failed: ${C_GREW[*]}"

...
# Unit isolation: cmd_loop_spawn normally shells out to the installed `5dive`
# binary to wake a real agent. That subprocess would reopen the live TASKS_DB,
# bypassing the temp paths above. Never dispatch from this harness.
TEST_AGENT="unit-loop-agent"
_loop_wake_agent() { :; }
...
out=$(run --role=maker --agent="$TEST_AGENT" --prompt="do a thing")
...
[[ "$tassignee" == "$TEST_AGENT" ]] && ok_t "backing task assigned to --agent" || bad_t "assignee" "$tassignee"

# --- T2: validation
run --agent="$TEST_AGENT" >/dev/null 2>&1; [[ $? -ne 0 ]] && ok_t "missing --prompt fails" || bad_t "missing prompt" "exit 0"
run --prompt=x   >/dev/null 2>&1; [[ $? -ne 0 ]] && ok_t "missing --agent fails"  || bad_t "missing agent" "exit 0"
run --agent="$TEST_AGENT" --prompt=x --role=bogus >/dev/null 2>&1; [[ $? -ne 0 ]] && ok_t "bad --role rejected" || bad_t "bad role" "exit 0"
run --agent="$TEST_AGENT" --prompt=x --ceiling=abc >/dev/null 2>&1; [[ $? -ne 0 ]] && ok_t "non-int --ceiling rejected" || bad_t "bad ceiling" "exit 0"
run --agent="$TEST_AGENT" --prompt=x --schema='{bad' >/dev/null 2>&1; [[ $? -ne 0 ]] && ok_t "invalid --schema json rejected" || bad_t "bad schema" "exit 0"

# --- T3: ceiling default resolution
out=$(run --agent="$TEST_AGENT" --prompt=y); c=$(printf '%s' "$out" | jq -r '.data.ceiling')
[[ "$c" == "200000" ]] && ok_t "built-in ceiling default applied ($c)" || bad_t "ceiling default" "$c"
out=$(LOOP_CEILING_DEFAULT=50000 run --agent="$TEST_AGENT" --prompt=y); c=$(printf '%s' "$out" | jq -r '.data.ceiling')
[[ "$c" == "50000" ]] && ok_t "env LOOP_CEILING_DEFAULT override ($c)" || bad_t "env ceiling" "$c"
out=$(run --agent="$TEST_AGENT" --prompt=y --ceiling=12345); c=$(printf '%s' "$out" | jq -r '.data.ceiling')
...
( cmd_loop_spawn --agent="$TEST_AGENT" --prompt="UNIQ_killme" --wait=20 >"$TMP"/loop-kill.out 2>&1 ) &
bgpid=$!
...
( cmd_loop_spawn --agent="$TEST_AGENT" --prompt="UNIQ_spendy" --ceiling=1000 --wait=20 >"$TMP"/loop-ceil.out 2>&1 ) &
bgpid=$!
...
( cmd_loop_spawn --agent="$TEST_AGENT" --prompt="UNIQ_finish" --wait=20 >"$TMP"/loop-done.out 2>&1 ) &
bgpid=$!

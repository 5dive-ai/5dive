...
# Unit isolation: batch spawns must not shell out to the installed heartbeat
# dispatcher, which would reopen the live task database in a subprocess.
TEST_AGENT="unit-batch-agent"
_loop_wake_agent() { :; }
...
( cmd_loop_map --agent="$TEST_AGENT" --do=x >/dev/null 2>&1 ); [[ $? -ne 0 ]] && ok_t "map: missing --over fails" || bad_t "map over" "exit 0"
( cmd_loop_map --agent="$TEST_AGENT" --over='not-json' --do=x >/dev/null 2>&1 ); [[ $? -ne 0 ]] && ok_t "map: non-array --over rejected" || bad_t "map array" "exit 0"
( cmd_loop_until_dry --agent="$TEST_AGENT" --round=x >/dev/null 2>&1 ); [[ $? -ne 0 ]] && ok_t "until-dry: missing --dedup-key fails" || bad_t "ud key" "exit 0"
...
simulate & SIMPID=$!
out=$(cmd_loop_map --agent="$TEST_AGENT" --role=worker --do='handle ITEM={}' --over='[10,20,30]' --timeout=30 2>"$TMP"/map.err)
...
out=$(cmd_loop_map --agent="$TEST_AGENT" --do='x ITEM={} FAILME-IF-2' --over='[1,2,3]' --timeout=30 2>/dev/null)
...
out=$(cmd_loop_map --agent="$TEST_AGENT" --do='ITEM={}' --over='[1,2]' --max-concurrency=999 --timeout=30 2>/dev/null)
...
echo 1 > "$TMP/roundctr"
out=$(cmd_loop_until_dry --agent="$TEST_AGENT" --round='ROUND_FINDER sweep' --dedup-key=id --stop-after=2 --round-timeout=30 2>"$TMP"/ud.err)
...
out=$(cmd_loop_until_dry --agent="$TEST_AGENT" --round='ROUND_FINDER sweep' --dedup-key=id --stop-after=5 --max-iters=2 --round-timeout=30 2>/dev/null)
...
h1=$(cmd_loop_spawn --agent="$TEST_AGENT" --prompt="COLLECT_A" 2>/dev/null | jq -r '.data.loopId')
h2=$(cmd_loop_spawn --agent="$TEST_AGENT" --prompt="COLLECT_B" 2>/dev/null | jq -r '.data.loopId')

...
plant() {
  local id="$1"
  ( cmd_task_need "$id" --type=decision --options="ship it now|hold it" --recommend="ship it now" \
...
( cmd_task_need "$t5" --type=decision --options="ship it now|hold it" --ask="first gate" --tier=1 ) >/dev/null 2>&1
if [[ "$(hist_n "$t5")" == "0" ]]; then

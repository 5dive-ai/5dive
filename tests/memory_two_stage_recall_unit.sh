...
# drop is the exact defect this row exists to remove. There are TWO branches and
# they must be armed separately: a slug with no room left is SKIPPED, a slug with
# partial room is cut short and must SAY so. An arm matching `TRUNCATED|SKIPPED`
# is satisfied by the SKIPPED branch alone, so emptying the truncation note left
# 31/31 green (quinn, iteration 1).
g5="$(_memory_get alpha_widget beta_relay gamma_gate --roots="$STORE" --max-tokens=30 2>&1)"
grep -q 'SKIPPED' <<<"$g5" \
  && ok_t "get: a slug with no room left is announced as SKIPPED" \
  || bad_t "get: ceiling dropped content silently" "$g5"

# The TRUNCATED branch needs room to be partially filled: one atom whose body is
# far over the ceiling, fetched alone, so `used` is 0 and the remainder is wide.
big="$(printf 'lorem ipsum dolor sit amet consectetur %.0s' $(seq 1 200))"
mk big_atom project "A deliberately oversized atom for the ceiling arm" "$big"
g6="$(_memory_get big_atom --roots="$STORE" --max-tokens=200 2>&1)"
grep -q 'TRUNCATED at the 200-token ceiling' <<<"$g6" \
  && [ "$(wc -c <<<"$g6")" -lt "$(wc -c <<<"$big")" ] \
  && ok_t "get: a body cut short by the ceiling says TRUNCATED and is actually shorter" \
  || bad_t "get: truncation was silent or did not shorten" "$(head -c 300 <<<"$g6")"
...
# The exemption must be NARROW as well as alive. It is keyed on a marker only the
# generator emits; keying it on prose ("5dive memory router" appearing anywhere in
# the file) let a genuinely FLAT index disarm the check for its whole store just by
# quoting the router's own instructions — which is exactly the sentence agents are
# told to paste into a keep block. The control above cannot reach this: its fixture
# never mentions the verb, so it proves the check is alive, not that it is narrow.
printf '# Index\n- [one](doc_atom_1.md) — hook\n\nRemember to run 5dive memory router --write some day.\n' > "$DOC/MEMORY.md"
proseout="$(_memory_scan_json "$TMP/no-code-root" "$DOC")"
grep -q 'index-drift' <<<"$proseout" \
  && ok_t "control: a FLAT index that merely mentions the router verb still reports index-drift" \
  || bad_t "the exemption is keyed on prose — any index naming the verb loses the check" "$proseout"
_memory_router --root="$DOC" --recent=2 --write >/dev/null 2>&1

...
#
# 91 -> 92 (DIVE-3474, merge of origin/main b898240 into dive-3474-a2a-wake):
# +gate_urgent. THIS IS THE COLLISION THE PARAGRAPHS ABOVE PREDICT, and it is the
# first time it has been caught by something other than this literal. Both sides
# independently moved the SAME literal 90 -> 91 — DIVE-3483 for stranded_pinged_at
# on main, DIVE-3474 for gate_urgent on the branch — and each control run was
# correct against its own base. The merged tree has BOTH columns, so 91 is wrong on
...
# adding four.
#
# 96 -> 98 (DIVE-4111): +reap_escalated_at/_n, the latch that makes the heartbeat's
# hard-cap auto-pause fire ONCE per (row, owner) instead of on every reap past the
# threshold. READ off this tree the way this file instructs — `bad` patched to print
...
# breaking: it is a CENSUS, so it is invalidated by an ADDED column exactly as
...
# offer's deadline). Anyone adding a column lands here, which is the point — the
# schema and its migration twin are two writes, and this arm is what proves the
# fresh-store half actually produced them.
[[ $rc -eq 0 && "$actual" == "$required" && "$column_count" == "115" ]] \
  && ok "fresh schema: all 115 columns, including the eight former holes, are present" \

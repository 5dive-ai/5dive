  ...
  if [[ "$verb" == "done" ]]; then
    ...
    if [[ -n "$_dref" || -n "$_branch" ]]; then
      ...
      if [[ -n "$_dref" ]]; then
        ...
        _state=$(gh pr view "$_dref" --json state,mergedAt -q '.state' 2>/dev/null || echo "")
        _merged=$(gh pr view "$_dref" --json state,mergedAt -q '.mergedAt' 2>/dev/null || echo "")
        ...
        local _bmerged
        _bmerged=$(gh pr list --head "$_branch" --state merged --json number,mergedAt -q '.[0].mergedAt' 2>/dev/null || echo "")
        if [[ -z "$_bmerged" || "$_bmerged" == "null" ]]; then
          fail "$E_CONFLICT" "$ident cannot close: no MERGED PR found for its branch '$_branch'. done=merged-to-main (DIVE-1830) — open and merge a PR for that branch, then run task done. Use \`task cancel\` to abandon."
        fi
      fi
    fi
  fi

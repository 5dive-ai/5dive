...
_gate_pr_refs_from_text() {
  local text="$1"
  {
    printf '%s' "$text" | grep -oP 'https://github\.com/[A-Za-z0-9._-]+/[A-Za-z0-9._-]+/pull/\K[0-9]{1,6}'  || true
    printf '%s' "$text" | grep -oiP '(?<![A-Za-z0-9])(?:PRs?|pull request)\s*#?\s*\K[0-9]{1,6}(?![0-9])'    || true
  } | awk 'NF && !seen[$0]++'
}
...
_task_status_cmd() {
  ...
  if [[ "$verb" == "done" ]]; then
    ...
    if [[ -z "$_dref" ]]; then
      local _body; _body=$(db "SELECT COALESCE(body,'') FROM tasks WHERE id=${id};")
    ...
    if [[ -n "$_dref" || -n "$_branch" ]]; then
      ...
      if [[ -n "$_dref" ]]; then
        ...
        # DIVE-1935: MERGED is not the same as GREEN. #156 was red on its own new
...
$(db "SELECT COALESCE(body,'') FROM tasks WHERE id=${id};")"
    ...
    if [[ -n "$_txt_red" && $force_merge_gate -eq 0 ]]; then
      policy_refuse "$E_CONFLICT" done-after-red-merge DIVE-1935 "$ident" "$ident cannot close: PR #${_txt_red//,/, #} named in its result/body is merged but its checks are RED. done=merged-AND-green (DIVE-1935) — fix main (or re-run the failed check), then task done, or \`task done $ident --force-merge-gate\` to override (audited)."
    fi
    if [[ $force_merge_gate -eq 1 ]]; then
...
_task_gate_escalate_via_sudo() {
  ...
  [[ -n "$cli" ]] || return 1
  printf '%s' "$nonce" | sudo -n "$cli" task gate-escalate "$ident" --nonce=- >/dev/null 2>&1
}
...
cmd_task_gate_escalate() {
  ...
  [[ $EUID -eq 0 ]] || fail "$E_PERMISSION" "task gate-escalate must run as root (it reads other agents' channel state)"
  ...
  audit_log "task gate-escalate" "error" 1 -- "task=$ident" "type=$nt" "filer=${filer:-unknown}" || true
...
task_need_notify() {
  ...
  if ! _task_owner_channel; then
    ...
    # the filer, so the chain must start from the gate row's own filer.

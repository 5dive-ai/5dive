...
5dive task — shared task queue (sqlite at ${STATE_DIR}/tasks/tasks.db)
  ...
  5dive task verifier <id|DIVE-N> <agent> [--accept=<criteria>] [--max-iters=<n>]
                                                     ...
                                                     # task (grader must differ from the assignee). The remedy when the
                                                     # DIVE-969 auto-skip declined the rail at filing time.
...
cmd_task_verifier() {
  ...
  local st kind title asignee cur_accept
  ...
  # writer != grader (DIVE-474): a maker grading itself is not a review.
  [[ "$who" != "$asignee" ]] \
    || fail "$E_VALIDATION" "'$who' is $ident's own assignee — a maker can't grade itself (reassign first, or pick a different grader)"
  ...
  db "UPDATE tasks SET verifier=$(sqlq "$who"),
      ...
      WHERE id=${id};"
  ok "$ident is now verifier-graded → $who ('task done' hands off to grade instead of closing)" \
     '{id:($i|tonumber), ident:$id, verifier:$v, assignee:$a, acceptanceCriteria:$ac}' \
     --arg i "$id" --arg id "$ident" --arg v "$who" --arg a "$asignee" --arg ac "$new_accept"
}

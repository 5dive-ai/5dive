...
task_need_notify() {
  ...
  # One message. Blank lines separate the header / ask / options / footer so a
  # long ask doesn't render as an unreadable wall on mobile. Dashboard is the
  # primary CTA (DIVE-104 live); CLI is a power tail. Options are listed one per
  # line (numbered to match the tap buttons) so long labels stay readable even
  # when Telegram crops the button text.

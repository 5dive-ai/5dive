...
cmd_task_need() {
  ...
  if [[ "$_nrc" == "2" ]]; then
    ...
    fail "$E_AUTH_REQUIRED" "$ident gate NOT filed — ${actor} has no paired channel and neither does anyone above it in the org chart, so this ask would reach no human. Pair a channel for ${actor}, or hand the ask to a paired agent: 5dive agent send <lead> \"...\". The task was left untouched."
  fi
...
_task_chain_paired() {
  ...
  while IFS= read -r c; do
    [[ -n "$c" ]] || continue
    if _task_agent_paired "$c"; then printf '%s' "$c"; return 0; fi

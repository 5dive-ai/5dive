...
_council_veto_ping() {
  ...
  # DIVE-1644: the notification must let the founder make an INFORMED veto call from the message
  # alone — so it now leads with WHAT carried (the motion/decision text) + the vote tally + any
  # dissent, then the sealed-receipt handle + hold deadline + the one-time nonce. The extra fields
  # are best-effort: a base-only offer (no motion/tally sourced) degrades to the receipt line so the
  # ping never regresses below the prior text.
  if [[ -z "${COUNCIL_MOCK:-}" ]] && command -v _tg_send >/dev/null 2>&1; then
    local msg="Council veto offer"
    [[ -n "$motion" ]]  && msg+=$'\n\nDecision: '"$motion"
    [[ -n "$tally" ]]   && msg+=$'\nVote: '"$tally"
    [[ -n "$dissent" && "$dissent" != "none" ]] && msg+=$'\nDissent: '"$dissent"
    msg+=$'\n\nA pass sealed ('"${digest:0:12}"$'…). Execution holds until '"$execute_after"$'. Tap to VETO (nonce '"$nonce"$'), or ignore to let it proceed.'
    _tg_send "$resolved" "$msg" >/dev/null 2>&1 || true
  fi
  return 0
}
    ...
    if [[ -n "$digest" ]]; then
      ...
      if [[ -w "$COUNCIL_RECEIPTS" ]]; then
        ...
        if [[ -n "$offer_resolved" ]]; then
          ...
          # founder ping is self-contained (no blind veto). tally renders as "carried A/T approve"
          # where T = approve+reject+escalate (total seats voting), plus the reject/escalate split.

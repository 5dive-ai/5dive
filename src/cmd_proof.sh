...
#   5dive proof tick        # cron driver (root); gated on the pref

_proof_pref_file() { echo "${STATE_DIR}/proof.json"; }
_PROOF_CRON="/etc/cron.d/5dive-proof"
...
# _proof_install_cron <hour> — idempotent root cron that runs `proof tick`
# daily at <hour>. Rewritten (never appended) so on/off/re-on is clean.
_proof_install_cron() {
  local hour="$1"
...
0 ${hour} * * * root /usr/local/bin/5dive proof tick >> /var/log/5dive-proof.log 2>&1
CRON
...
_proof_onoff() {
  ...
  local repo="" branch="" hour="" f; f="$(_proof_pref_file)"
  while [ $# -gt 0 ]; do
    case "$1" in
      --repo=*)   repo="${1#*=}" ;;
      --branch=*) branch="${1#*=}" ;;
  ...
  case "$sub" in
    on)
      ...
      mkdir -p "$(dirname "$f")"
      jq --arg r "$repo" --arg b "$branch" --argjson h "$hour" \
        '.enabled=true | .repo=$r | .branch=$b | .hour=$h' <<<"$cur" > "$f.tmp" && mv "$f.tmp" "$f"
      _proof_install_cron "$hour"
      echo "proof: ON — daily ${hour}:00 → ${repo} (${branch})"
      ;;
    off)
      require_root
    ...
    status)
      ...
      local enabled repo_c branch_c hour_c last today staleness
      ...
      if [ "$enabled" = "true" ]; then
        echo "proof: ON — daily ${hour_c}:00 → ${repo_c} (${branch_c})"
      elif [ -n "$repo_c" ]; then

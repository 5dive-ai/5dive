...
if not _ships.isdigit():
    ...
    metrics.append(metric("autonomous rollback rate", None,
                          nodata=("no ship recorded through `5dive push` in this window"
                                  + (f" (ledger opened {_since})" if _since else " (ledger is empty)")
                                  + " — a rate over zero ships is undefined, not 0.0% (DIVE-1923)")))
else:
    _s, _r, _u = int(_ships), int(_rb or 0), int(_rbu or 0)
    _sample = f"{_r} of {_s} recorded ships reverted"
    if _u:
    ...
    metrics.append(metric("autonomous rollback rate", f"{round(_r / _s * 100, 1)}%", _sample,
                          note="a ship is a commit pushed through `5dive push`; work landed by "
                               "any other route is invisible to this row"))

...
cmd_agent_rotation_rotate() {
  ...
  if [[ -z "$target" ]]; then
    if (( JSON_MODE )); then
      ...
      echo "OK — no eligible account to rotate to (all cooling / none configured)"
    fi
    return 0
  fi
  ...
  # stdout so we emit a single clean envelope.
  cmd_config "$name" set "auth-profile=${target}" >/dev/null
  ok "rotated '$name': ${current:--} -> ${target}" \
     '{rotated:true, from:$f, to:$t}' --arg f "$current" --arg t "$target"
}

...
# ---------------------------------------------------------------------------
# DIVE-4779 — THE PRODUCER MOVED TO `cmd_board.sh` AND THIS IS THE ONE LINE LEFT.
# It is not a tidy-up: the UI is leaving core (step 3 of DIVE-4779 removes this
# whole file behind the moved-verb shim), and the document it serves must NOT
# leave with it, because the document is a statement about core's own data model
# and the plugin cannot make one. So `/api/state` and `--data` now serve exactly
# what `5dive board` serves — one producer, one contract version, no possibility
# of the in-core UI and the plugin UI disagreeing about what a board is while
# both still exist. Deleting this file later changes nothing for the plugin.
_ui_state_json() { _board_state_json "$@"; }

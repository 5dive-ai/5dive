...
cmd_heartbeat_tick() {
  ...
  (( ${_PR_FIRED:-0} || ${_PR_DEFERRED:-0} || ${_PR_CLEARED:-0} || ${_PR_FAILED:-0} || ${_PR_PARKED:-0} )) \
    && _hb_log "[pending-restart] pass done — ${_PR_FIRED:-0} bounced, ${_PR_DEFERRED:-0} still deferred (${_PR_OVERDUE:-0} overdue), ${_PR_CLEARED:-0} cleared, ${_PR_FAILED:-0} failed, ${_PR_PARKED:-0} left parked (desiredState=stopped)"

...
5dive heartbeat — wake agents only when they have queued tasks
  ...
  5dive heartbeat wake-mode <name> [always_on|cold] [--cap=<n>]
                                          # DIVE-1858: opt-in reactive wake mode + wake-budget.
                                          # no args after <name> => show current mode/budget/cost.
  ...
  wake-mode (DIVE-1858 Phase 1): 'cold' opts an agent into reactive
        ...
        billing). 'always_on' (default) is unchanged. main + marketing are
        pinned always-on and refuse 'cold'. NOTE: live auto-sleep is Phase 2,
        held for lead review — Stage 1 only adds the flag + budget guardrail.
...
cmd_heartbeat_wake_mode() {
  ...
  [[ -n "$name" ]] || fail "$E_USAGE" "usage: 5dive heartbeat wake-mode <name> [always_on|cold] [--cap=<n>]"
  require_agent "$name"

  # Read-only path: no mode + no cap => print current state, no root needed.
  if [[ -z "$mode" && -z "$cap" ]]; then
    local reg cur jcap jused jday
    reg=$(registry_read)
    ...
    ok "wake mode for '$name': ${cur} (cap ${jcap}/day, used ${jused} on ${jday}; cost/wake ${_HB_WAKE_COST_EST})" \
       '{name:$n, mode:$m, capPerDay:$c, wakesToday:($u|tonumber), day:$d, costPerWake:$cost}' \
       --arg n "$name" --arg m "$cur" --arg c "$jcap" --arg u "$jused" --arg d "$jday" --arg cost "$_HB_WAKE_COST_EST"
    return 0
  fi

  require_root "heartbeat wake-mode"
  [[ -z "$mode" ]] && mode="cold"   # `--cap` alone implies opting into cold
  case "$mode" in
    always_on|cold) ;;
  ...
  with_registry_lock _hb_wake_mode_write "$name" "$mode" "$cap"
}
...
_hb_wake_mode_write() {
  ...
  local shown_cap; shown_cap=$(jq -r --arg n "$name" '.agents[$n].wake.budget.capPerDay // "-"' <<<"$reg")
  ok "wake mode for '$name' set to '$mode' (cap ${shown_cap}/day; cost/wake ${_HB_WAKE_COST_EST}; live sleep is Phase 2, held for lead review)" \
     '{name:$n, mode:$m, capPerDay:$c}' --arg n "$name" --arg m "$mode" --arg c "$shown_cap"
}
...
cmd_heartbeat_tick() {
  ...
  ok "heartbeat tick: woke ${woke} / reclaimed ${reclaimed} / reaped ${reaped} / starved ${starved} / spread-deferred ${sk_spread} / active-deferred ${sk_active} / budget-skipped ${sk_budget} / checked ${checked}" \
     ...
     --arg c "$checked" --arg w "$woke" --arg rc "$reclaimed" --arg r "$reaped" --arg st "$starved" --arg nd "$sk_notdue" --arg b "$sk_busy" --arg nw "$sk_nowork" --arg sp "$sk_spread" --arg ac "$sk_active" --arg bu "$sk_budget" --arg sf "$sk_fail"
}

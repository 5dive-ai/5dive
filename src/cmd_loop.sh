...
_loop_refresh_spend() {
  ...
  if (( prc != 0 )) || [[ ! "$spent" =~ ^[0-9]+$ ]]; then
    ...
    rm -f "$errf"
    _loop_spend_unreadable "$loop_id" "spend recompute failed (rc=${prc}, out='${spent:-}')${detail}"
    return 2
  fi
  rm -f "$errf"
  # Persist ONLY a read that succeeded — see the header: this UPDATE used to run
  # on the fail-open path and overwrite the last good total with a coerced 0.
...
_loop_spent() {
  ...
  if (( now - last >= throttle )); then
    _loop_refresh_spend "$loop_id" >/dev/null || rc=$?
    _LOOP_SPEND_LAST[$loop_id]=$now
  fi
  # DIVE-2304: propagate NOT-REACHED. Falling through to the SELECT here would
  # re-launder a failed refresh as the (now un-clobbered, but stale) persisted
  # number, which reads exactly like a fresh one to every caller.
  (( rc == 0 )) || return 2
  local v; v=$(db "SELECT COALESCE(tokens_spent,0) FROM loop_runs WHERE loop_id=$(sqlq "$loop_id");")
  [[ "$v" =~ ^[0-9]+$ ]] || { _loop_spend_unreadable "$loop_id" "persisted tokens_spent unreadable ('${v:-}')"; return 2; }
  printf '%s' "$v"
}

# _loop_ceiling_check <loop_id> <ceiling> — DIVE-2304: the three-state budget
# test every --wait poll goes through.
#   rc 0 -> under the ceiling; echoes the spend (safe to keep spending)
#   rc 1 -> at/over the ceiling; echoes the spend (halt reason "ceiling")
#   rc 2 -> spend NOT-REACHED; echoes nothing  (halt reason "spend-unreadable")
# The old `[[ "${spent:-0}" -ge "$ceiling" ]]` at each call site could not be
# fixed in place: an integer cannot carry a third state, so the consumer had no
# way to tell "0 spent" from "no idea". A budget control that cannot read the
# budget has not verified anything, so rc 2 halts rather than continues.
_loop_ceiling_check() {
  local loop_id="$1" ceiling="$2" spent rc=0
  spent=$(_loop_spent "$loop_id") || rc=$?
  (( rc == 0 )) || return 2
  [[ "$spent" =~ ^[0-9]+$ ]] || return 2
  printf '%s' "$spent"
  [[ -n "$ceiling" && "$ceiling" =~ ^[0-9]+$ ]] || return 0
  (( spent >= ceiling )) && return 1
  return 0
}
...
cmd_loop_spawn() {
  ...
  local final_status="running" tstatus="" tresult="" killed="" spent="0" crc=0
  while :; do
    local t; t=$(date +%s)
    ...
    # DIVE-2304: three states, not two. rc 2 = the spend could not be read at
    # all, which used to arrive here as a 0 and read as "plenty of budget left".
    if (( crc == 2 )); then final_status="spend-unreadable"; break; fi
    if (( crc == 1 )); then final_status="ceiling"; break; fi
  ...
  case "$final_status" in
    ...
    ceiling|spend-unreadable|escalated|rejected|timeout) loop_status="escalated" ;;
  esac
...
cmd_loop_verify() {
  ...
  local final_status="running" killed="" spent="0" crc=0
  while :; do
    local t; t=$(date +%s)
    killed=$(db "SELECT kill_requested FROM loop_runs WHERE loop_id=$(sqlq "$loop_id");")
    if [[ "$killed" == "1" ]]; then final_status="killed"; break; fi
...
cmd_loop_map() {
  ...
  local -a child_ids=()
  local next=0 completed=0 ok_n=0 failed_n=0 live=0 halt="" spent="0" crc=0
...
cmd_loop_until_dry() {
  ...
  local -a child_ids=()
  local round_no=0 empty_streak=0 exit_reason="" spent="0" crc=0
  ...
  while :; do
    ...
    spent=$(_loop_ceiling_check "$loop_id" "$eff_ceiling") && crc=0 || crc=$?
    (( crc == 2 )) && { exit_reason="spend-unreadable"; break; }   # DIVE-2304
    (( crc == 1 )) && { exit_reason="ceiling"; break; }

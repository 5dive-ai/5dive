...
cmd_objective_status() {
  ...
  local verified originated_open
  ...
  if (( JSON_MODE )); then
    ...
    jq -cn \
      ...
      --argjson verified "${verified:-0}" --argjson open "${originated_open:-0}" \
      ...
      '{ok:true, data:{objective:$name, status:$status, mode:$mode,
        ...
        verified_this_cycle:$verified, originated_open:$open,
  ...
  printf 'verified this cycle: %s   (originated open: %s)\n' "${verified:-0}" "${originated_open:-0}"

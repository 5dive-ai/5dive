...
_prod_registry = os.environ.get("USAGE_PROD_REGISTRY") or "/var/lib/5dive/agents.json"
try:
    _is_prod_store = os.path.realpath(registry) == os.path.realpath(_prod_registry)
except Exception:
    _is_prod_store = False
...
def home_of(name):
    ...
    # DIVE-2069 — FAIL CLOSED. This fallback used to resolve the REAL agent homes
    # unconditionally, so a harness whose fixtures landed anywhere else did not fail:
    # it silently read the fleet's live transcripts and scored partial marks off them
    # (721 files under /home/agent-dev alone). Read-only, so nothing was corrupted —
    # but "this harness passes" became partly a statement about production data, and
    # that is unfalsifiable from inside the harness, because a real transcript tree
    # looks exactly like a well-seeded one.
    #
    # Fenced on STORE IDENTITY, not on a test marker, and the distinction is the whole
    # point (DIVE-1968's lesson, same shape): NONE of the three harnesses that call
    # usage_collect sets FIVEDIVE_TEST, so a marker-keyed guard would never have fired
    # — including on the harness that had the bug. An opt-in fence is a fence you have
    # to REMEMBER. Store identity needs no env at all, so a harness that sets nothing
    # is fenced BY CONSTRUCTION, including ones nobody has written yet.
    #
    # A fixture registry naming agents whose PRODUCTION homes we would then read is a
    # category error, not a fallback: fixture roster, real transcripts. Refuse loudly
    # and name the remedy. Production is unaffected — it reads the prod registry, so
    # _is_prod_store is true and the fallback behaves exactly as before.
    if not _is_prod_store:
        sys.exit(
            "usage_collect: refusing to read real agent homes (/home/agent-*) while "
            "running against a NON-PRODUCTION registry (%s). That would attribute the "
            "fleet's live transcripts to a fixture roster. Set USAGE_HOME_ROOT to a "
            "throwaway tree and seed <root>/agent-<name>/.claude/projects/... "
            "(DIVE-2069)." % registry
        )

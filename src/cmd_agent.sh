  ...
  if (( JSON_MODE )); then
    ...
    jq -r --arg authLine "$_auth_line" '
      ...
      (if .sudo.diverges then
         "\nWARNING: the label and the enforced grant DISAGREE. Label \"\(.isolation)\" describes \(if .isolation == "beyond-admin" then "any command as any user" elif .isolation == "admin" then "the 5dive CLI as root" elif .isolation == "standard" then "a few named 5dive subcommands only" else "no sudo" end); the enforced grant is \(.sudo.grant) (\(.sudo.scope), runas \(.sudo.runas)). Trust the grant, not the label. Install/upgrade reconciles only clean 5dive-managed standard grants; custom, missing, admin and root-all policies stay untouched and need an operator decision."
       else empty end)
    ' <<<"$obj"
  fi
}

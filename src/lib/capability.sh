...
capability_reverify_from_sudoers() {
  ...
  [[ -n "$user" ]] || return 2
  # NOT one `local` line: the builtin expands every word BEFORE it assigns any,
  # so `local user="$1" f=".../${user}"` reads $user while it is still unset and
  # dies under `set -u`. Found by the unit arm, in this function, before it ran
  # anywhere real.

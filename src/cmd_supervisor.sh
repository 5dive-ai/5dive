...
Classification (conservative — see docs/fleet-supervisor-design.md §4):
  ...
  stuck    service/tmux/poller dead, a loop self-flagged stuck, no progress for
           ${_SUP_T_STUCK_MIN}m+ with active work, or the box CLI is stale
           (cause: service-dead|tmux-dead|poller-dead|loop-stuck|no-progress|stale-cli)
...
_sup_agent_record() {
  local name="$1" type="$2" channels="$3" unit="$4" user="$5" sess="$6" home="$7" now="$8"
  ...
  if (( ! svc_running )); then
    if [[ "$active" == "failed" ]] || (( has_work )); then
      class="stuck"; cause="service-dead"; detail="unit ${active:-unknown}"
    else
    ...
    # confirmed probe; "unknown" never lands here.
    class="stuck"; cause="stale-cli"; detail="box CLI ${FIVE_VERSION} stale behind ${_SUP_CLI_LATEST}"
...
_sup_snapshot() {
  ...
  for name in $(jq -r '.agents | keys[]' <<<"$reg" 2>/dev/null); do
    ...
    rows+=$(_sup_agent_record "$name" "$type" "$channels" \
      "5dive-agent@${name}.service" "agent-${name}" "agent-${name}" "/home/agent-${name}" "$now")
    rows+=$'\n'
  done
...
cmd_supervisor_tick() {
  ...
  # ── P2 GOES HERE: the recovery ladder (ACT + ESCALATE, design §5–6) ─────────
  # nudge -> resume -> rotate -> restart, lowest rung matching the cause, with
  # exponential backoff + max attempts + a full audit row per action, and
  # escalate-not-act for anything irreversible (reprovision stays manual
  # forever). P1 is observe-only BY SCOPE (owner-locked): do not add actions
  # here without the P2 sign-off.
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  ok "supervisor tick: ${total} agents — ${healthy} healthy / ${slow} slow / ${stuck} stuck · ${events} audit row(s)" \
     '{enabled:true, agents:($t|tonumber), healthy:($h|tonumber), slow:($sl|tonumber), stuck:($st|tonumber), auditRows:($e|tonumber)}' \

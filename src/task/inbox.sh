...
cmd_task_inbox() {
  ...
  if (( JSON_MODE )); then
    ...
    rows=$(dbfmt -json "SELECT id, ident, title, status, priority, assignee, created_by, parent_id, created_at, need_type, ask, need_options, recommend, tier, precedent_ref, need_answer, need_answered_at, need_expires_at, gate_pinged_at FROM tasks WHERE ${where} ${order};")
    [[ -n "$rows" ]] || rows="[]"

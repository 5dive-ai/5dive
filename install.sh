...
if [[ "${1:-}" == "--uninstall" ]]; then
  ...
  if [[ -z "$GH_PINNED_SHA" ]]; then
    GH_PINNED_TAG=""; _rgt_rc=0
    GH_PINNED_TAG="$(resolve_gh_tag)" || _rgt_rc=$?
    if [[ -z "$GH_PINNED_TAG" ]]; then
      ...
      case "$_rgt_rc" in
        2)
          ...
          ;;
        3)
          printf 'error: 5dive install: EVERY RELEASE TAG IS HELD — refusing to install a held release.\n' >&2
          printf '       A hold is armed on main against every candidate release. This is deliberate; ask whoever armed it.\n' >&2
          ;;
        *)
